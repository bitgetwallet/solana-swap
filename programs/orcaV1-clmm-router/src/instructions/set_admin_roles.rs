...
pub fn set_protocol_program_id(ctx: Context<SetAdminRole>, protocol_program_id: Pubkey) -> Result<()> {
    ...
    let account = &mut ctx.accounts.admin_info;
    require!(protocol_program_id.key() != account.protocol_program_id.key(),ErrorCode::ValueCannotBeEqual);

    msg!("old protocol_program_id is {:?}", account.protocol_program_id);
    account.protocol_program_id = protocol_program_id;
    msg!("new protocol_program_id is {:?}", account.protocol_program_id);
