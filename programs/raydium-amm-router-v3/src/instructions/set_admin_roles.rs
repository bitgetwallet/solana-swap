...
pub fn set_amm_program_id(ctx: Context<SetAdminRole>, amm_program_id: Pubkey) -> Result<()> {
    require!(amm_program_id != Pubkey::default(), ErrorCode::AddressCannotBeNull);
    let account = &mut ctx.accounts.admin_info;
    require!(amm_program_id.key() != account.amm_program_id.key(),ErrorCode::ValueCannotBeEqual);

    msg!("old amm_program_id is {:?}", account.amm_program_id);
    account.amm_program_id = amm_program_id;
    msg!("new amm_program_id is {:?}", account.amm_program_id);
