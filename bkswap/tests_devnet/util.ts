import {InstructionType} from '@raydium-io/raydium-sdk/src/base/type';
import {
  Account,
  Connection,
  Keypair,
...
export async function createMintPair(provider: anchor.AnchorProvider) {
  ...
  tx2.add(
    Token.createMintToInstruction(TOKEN_PROGRAM_ID, tokenAMintAddress, associatedTokenA, wallet.publicKey,
      // @ts-ignore
      [wallet.payer], 100000000000000),
    Token.createMintToInstruction(TOKEN_PROGRAM_ID, tokenBMintAddress, associatedTokenB, wallet.publicKey,
      // @ts-ignore
      [wallet.payer], 200000000000000)
  );
